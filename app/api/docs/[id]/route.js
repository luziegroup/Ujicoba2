...
export async function POST(req) {
  ...
  const { title, content, passThreshold, manualQuiz, originalLink } = await req.json();
  if (!title || !content) {
  ...
  try {
    const pool = getPool();
    const { rows } = await pool.query(
      `insert into documents (id, title, doc_date, summary, content, quiz, pass_threshold, original_link)

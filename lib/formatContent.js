// Teks hasil ekstraksi PDF/upload sering jadi SATU baris panjang tanpa
// baris baru sama sekali, sehingga tanda "- item" yang aslinya daftar
// tidak bisa dikenali. Fungsi ini menyisipkan baris baru sebelum pola
// "- Kalimat" (tanda hubung diikuti spasi lalu huruf kapital/angka),
// supaya strukturnya bisa dikenali baik oleh AI maupun mode cadangan.
function preprocessRawContent(raw) {
  return (raw || "").replace(/\s[-–—]\s(?=[A-Z0-9])/g, "\n- ");
}
...
export async function formatDocumentHtml(title, rawContent) {
  const preprocessed = preprocessRawContent(rawContent);
